    ...
    ui(new Ui::MultiSendForm)
{
    ...
    ui->comboBox_IniFile->setItemDelegate(new QStyledItemDelegate(ui->comboBox_IniFile));
...
void MultiSendForm::UiInit()
{
    for (int i = 0; i < mulSendItemMaxNum; ++i) {
        ...
        item.btn->setObjectName("primaryButton");
        item.btn->setMinimumWidth(50);
        item.indexEdit = new QLineEdit(QString("0"));
        item.indexEdit->setMaximumWidth(20);
        item.indexEdit->setMinimumWidth(20);
        item.timerEdit = new QLineEdit(QString("1000"));
        item.timerEdit->setMaximumWidth(50);
        item.indexEdit->setMinimumWidth(50);
